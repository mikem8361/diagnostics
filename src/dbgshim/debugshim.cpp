    ...
    CLR_DEBUGGING_PROCESS_FLAGS * pFlags)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = GetCLRInfo(pDt, moduleBaseAddress, clrInfo);
    }
    ...
    {
        memcpy(pVersion, &clrInfo.Version, sizeof(CLR_DEBUGGING_VERSION));
    }
...
HRESULT CLRDebuggingImpl::ProvideLibraries(
    ...
    HMODULE* phDac)
{
    ICLRDebuggingLibraryProvider2* pLibraryProvider2 = NULL;
    ICLRDebuggingLibraryProvider3* pLibraryProvider3 = NULL;
    LPWSTR pDbiModulePath = NULL;
    LPWSTR pDacModulePath = NULL;
    HRESULT hr = S_OK;

    _ASSERTE(pLibraryProvider != NULL);
    _ASSERTE(phDbi != NULL);
    _ASSERTE(phDac != NULL);

    if (!clrInfo.WindowsTarget && SUCCEEDED(pLibraryProvider->QueryInterface(__uuidof(ICLRDebuggingLibraryProvider3), (void**)&pLibraryProvider3)))
    {
        // Ask library provider for DBI
        if (FAILED(pLibraryProvider3->ProvideUnixLibrary(clrInfo.DbiName, clrInfo.DbiBuildIdSize > 0 ? clrInfo.DbiBuildId : NULL, clrInfo.DbiBuildIdSize, &pDbiModulePath)) || pDbiModulePath == NULL)
        {
            hr = CORDBG_E_LIBRARY_PROVIDER_ERROR;
            goto exit;
        }
        // Ask library provider for DAC
        if (FAILED(pLibraryProvider3->ProvideUnixLibrary(clrInfo.DacName, clrInfo.DacBuildIdSize > 0 ? clrInfo.DacBuildId : NULL, clrInfo.DacBuildIdSize, &pDacModulePath)) || pDacModulePath == NULL)
        {
            hr = CORDBG_E_LIBRARY_PROVIDER_ERROR;
            goto exit;
        }
    }
    else if (SUCCEEDED(pLibraryProvider->QueryInterface(__uuidof(ICLRDebuggingLibraryProvider2), (void**)&pLibraryProvider2)))
    {
        // Ask library provider for DBI
...
exit:
    ...
    if (pLibraryProvider3 != NULL)
    {
        pLibraryProvider3->Release();
    }
    if (pLibraryProvider2 != NULL)
    {
                                     ...
                                     ClrInfo& clrInfo)
{
    ...
    if (IsTargetWindows(pDataTarget))
    {
        ...
        if (SUCCEEDED(hr))
        {
            clrInfo.Version.wMajor = (WORD) (fixedFileInfo.dwProductVersionMS >> 16);
            clrInfo.Version.wMinor = (WORD) (fixedFileInfo.dwProductVersionMS & 0xFFFF);
            clrInfo.Version.wBuild = (WORD) (fixedFileInfo.dwProductVersionLS >> 16);
            clrInfo.Version.wRevision = (WORD) (fixedFileInfo.dwProductVersionLS & 0xFFFF);
        }
