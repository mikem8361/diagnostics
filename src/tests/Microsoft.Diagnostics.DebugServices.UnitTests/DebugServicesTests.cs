...
{
    public class DebugServicesTests
    {
        ...
        {
            _configurations ??= TestRunConfiguration.Instance.Configurations.Select((config) => TestHost.CreateHost(config)).Select((host) => new[] { host }).ToImmutableArray();
        ...
        public void TargetTests(TestHost host)
        {
            ...
            Assert.NotNull(target.Host.CurrentTarget);
        ...
        public void RuntimeTests(TestHost host)
        {
            ...
            Assert.NotNull(runtimeService.CurrentRuntime);
